...
/* monitors */
/* mfact: sets the factor of master area size compared to stack area * this is a float value in the range of 0.05 to 0.95 * if set to -1 then the default value as set in config.h is used. */
/*        split workspace by factor when creating new windows (see setmfact) */
/* nmaster: sets the number of clients in the master area * if set to -1 then the default value as set in config.h is used */
...
static const double accel_speed = 1.5;
...
static const char *volupcmd[] = { "wpctl", "set-volume", "-l", MAX_VOLUME_MULTIPLIER(1.5), "@DEFAULT_AUDIO_SINK@", "5%+", NULL };
